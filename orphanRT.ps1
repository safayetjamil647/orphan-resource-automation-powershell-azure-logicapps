...
$Results = @()
$Query = "resources| where type == 'microsoft.network/routetables'| where isnull(properties.subnets)| project Resource=id, resourceGroup, subscriptionId, location"
...
$Collections | Export-Csv .\report\orphanRTList.csv -NoTypeInformation
