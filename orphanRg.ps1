...
$Results = @()
$Query = "ResourceContainers| where type == 'microsoft.resources/subscriptions/resourcegroups'| extend rgAndSub = strcat(resourceGroup, '--', subscriptionId)| join kind=leftouter (Resources| extend rgAndSub = strcat(resourceGroup, '--', subscriptionId)| summarize count() by rgAndSub) on rgAndSub| where isnull(count_)| project Resource=id, resourceGroup, subscriptionId, location"
...
$Collections | Export-Csv .\report\orphanRgList.csv -NoTypeInformation
