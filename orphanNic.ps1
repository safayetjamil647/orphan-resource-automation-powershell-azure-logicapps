...
$Results = @()
$Query = "Resources| where type has 'microsoft.network/networkinterfaces'| where isnull(properties.privateEndpoint)| where isnull(properties.privateLinkService)| where properties.hostedWorkloads == '[]'| where properties !has 'virtualmachine'| project Resource=id, resourceGroup, subscriptionId, location"
...
$Collections | Export-Csv .\report\orphanNicList.csv -NoTypeInformation
