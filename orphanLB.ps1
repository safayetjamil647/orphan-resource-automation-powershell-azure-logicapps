...
$Results = @()
$Query = "resources| where type == 'microsoft.network/loadbalancers'| where properties.backendAddressPools == '[]'| project Resource=id, resourceGroup, subscriptionId, location"
...
$Collections | Export-Csv .\report\orphanLBList.csv -NoTypeInformation
